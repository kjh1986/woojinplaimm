...
<script>
	$(document).ready(function() {
		if("${code}"=="100"){
			window.alert("과정구분입력이 완료 되었습니다.");
		}else if("${code}"=="101"){
			window.alert("과정구분입력이 실패했습니다.");
		}else if("${code}"=="200"){
			window.alert("과정구분수정이 완료 되었습니다.");
		}else if("${code}"=="201"){
			window.alert("과정구분수정이 실패했습니다.");
		}else if("${code}"=="300"){
			window.alert("과정구분삭제가 완료 되었습니다.");
		}else if("${code}"=="301"){
			window.alert("과정구분삭제가 실패했습니다.");
		}
		
		//수정액션리스너 
		$("button.btn-ocdInfo").on("click",function(){
			var ocd_code = $(this).attr("value");
			var ocd_name = $(this).parents("tr").children(":nth-child(2)").text();
			
			$("#infoModal input#ocd_code").val(ocd_code);
			$("#infoModal input[name=ocd_code]").val(ocd_code);
			$("#infoModal input#ocd_name").val(ocd_name);
			$("#infoModal").modal();
		});

		//삭제액션리스너 
		$("button.btn-ocdDelete").on("click",function(){
			var ocd_code = $(this).attr("value");
			var ocd_name = $(this).parents("tr").children(":nth-child(2)").text();
			
			$("#deleteModal input#ocd_code").val(ocd_code);
			$("#deleteModal input[name=ocd_code]").val(ocd_code);
			$("#deleteModal input#ocd_name").val(ocd_name);
		...
		<div class="row content">
			<div class="col-md-2 sidenav" style="padding: 0px;">
				<div class="list-group">
					...
					<div class="list-group collapse in" id="basicCallapse">
						...
						<a class="list-group-item active" href="${pageContext.request.contextPath}/basicInfo/ocdList">개설과정구분</a>
			...
			<div class="col-md-10 text-left" style="padding: 0px;"
				...
				<div class="panel panel-info">
					<div class="panel-heading">개설과정구분목록</div>
					<div class="panel-body">
							...
							<tbody>
								<c:forEach var="bi" items="${tableData}">
									<tr>
										<td>${bi.ocd_code}</td>
										<td>${bi.ocd_name}</td>
										<td><button class="btn btn-default btn-sm btn-ocdInfo" value="${bi.ocd_code}">수정</button></td>
										<td><button class="btn btn-danger btn-sm btn-ocdDelete"
												value="${bi.ocd_code}">삭제</button></td>
						...
						<form>
							<select class="form-control" name="key" id="key">
								<option value="ocd_code">구분번호</option>
		...
		<div class="modal-dialog">
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title">과정구분정보</h4>
				</div>
				<div class="modal-body">
					<form action="${pageContext.request.contextPath}/basicInfo/ocdUpdate" method="POST" class="form-inline" id="courseInfo">
						<div class="row">
							<div class="col-md-4" style="text-align: right">
								<label for="ocd_code">No : </label><br>
								<label for="ocd_name">과정구분명 : </label><br>
							</div>
							<div class="col-md-6">
								<input type="text" class="form-control" id="ocd_code" disabled>
								<input type="hidden" name="ocd_code">
								<input type="text" class="form-control" name="ocd_name" id="ocd_name">
				...
				<div id="deleteModal" class="modal fade" role="dialog">
					<div class="modal-dialog">
						...
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal">&times;</button>
								<h4 class="modal-title">과정구분삭제</h4>
							</div>
							<div class="modal-body">
								<form action="${pageContext.request.contextPath}/basicInfo/ocdDelete" method="POST"
									class="form-inline">
									<div class="row">
										<div class="col-md-4" style="text-align: right">
											<label for="ocd_code">No : </label> <br> <label
												for="ocd_name">구분명 : </label><br>
										</div>
										<div class="col-md-6">
											<input type="text" class="form-control" id="ocd_code"
												disabled> <input type="hidden"
												name="ocd_code"> <input type="text"
												class="form-control" name="ocd_name"
												id="ocd_name" disabled>
										</div>
									</div>
									<hr>
									<div style="text-align: right">
				...
				<div id="insertModal" class="modal fade" role="dialog">
					<div class="modal-dialog">
						...
						<div class="modal-content">
							<div class="modal-header">
								<button type="button" class="close" data-dismiss="modal">&times;</button>
								<h4 class="modal-title">과정구분등록</h4>
							</div>
							<div class="modal-body">
								<form class="form-inline" method="POST"
									action="${pageContext.request.contextPath}/basicInfo/ocdInsert">
									<div class="row">
										<div class="col-md-4" style="text-align: right">
											<label for="ocd_name">구분명 : </label>
										</div>
										<div class="col-md-6">
											<input type="text" class="form-control"
												name="ocd_name" id="ocd_name">
										</div>
									</div>
									<hr>
